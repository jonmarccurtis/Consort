...
class CuTable
{
    ...
    {
        wp_enqueue_script('cu_tablesorter');
    ...
    public function get_html()
    {
        ...
        <div class="tborder topic_table' . ($this->options['no select'] ? ' noselect' : '') . '">
			<table id="cu_table" class="cu_table table_grid ' . $this->table_class . '" cellspacing="0" width="100%"
			    style="table-layout:' . $this->options['table-layout'] . '">
			<caption class="cu_caption">';

        $html .= '<div class="header">
            <div class="header-left">';

        if (!empty($this->options['title'])) {
            $html .= '<span class="cu_title"> ' . $this->options['title'] . '</span> &nbsp; ';
        }
        ...
        if ($this->options['hide columns']) {
            $html .= '
                    <li class="hide_button"><a href="#" onclick="cu_table.hide_columns()" title="Hide/Unhide columns"><span class="hide_button_text">HIDE COLUMNS</span></a></li>';
        }
        if (!empty($this->options['download'])) {
            $html .= '
                    <li><a href="#" onclick="cu_table.download()" title="Download visible table to CSV"><span>DOWNLOAD</span></a></li>';
        }
    ...
    private function getJS()
    {
        ...
        if (!empty($this->presets)) {
            $js .= '
            $("#cu_table").bind("filterEnd sortEnd", function(e, table) {
                var data = {};
        ...
        $js .= '
            $("#cu_table").bind("tablesorter-initialized", function(e, table) {';
                        ...
                        $(".tablesorter-filter").each(function(i, filter) {
                            ...
                            if (column > 1) {
                                if (data["filters"][column] == "")
                                    $(filter).removeClass("cu_filtered");
                                else
                                    $(filter).addClass("cu_filtered");
                            }
                        });';
            }
            $js .= '
            ...
            $js .= '
                $(".tablesorter-filter").change(function() {
                    if ($(this).data("column") > 1) {
                        if ($(this).val() == "") 
                            $(this).removeClass("cu_filtered");
                        else
                            $(this).addClass("cu_filtered");
                    }
                });';
        }
        ...
        if ($this->options['hide columns']) {
            ...
            function hide_columns() {
                ...
                if ("HIDE SELECTED COLUMNS" == button_text.html()) {
                    ...
                    var hidden = false;
                    $.each($("table#cu_table-sticky").find("input[name=\'hide_check\']:checked"), function() {
                        var col = $(this).attr("col-number");
            ...
            cu_table.hide_columns = hide_columns;';
        }
